...
class BeamInput(BaseModel):
    ...
    point_load_n: float | None = Field(default=None, ge=0)
    point_load_position_m: float | None = Field(default=None, ge=0)
    udl_n_per_m: float | None = Field(default=None, ge=0)
    ...
    def validate_case_loads(self) -> "BeamInput":
        if "point" in self.case:
            ...
            if self.point_load_n <= 0:
                raise ValueError("point_load_n must be > 0 for point-load cases")
        ...
        if "udl" in self.case:
            if self.udl_n_per_m is None:
                raise ValueError("udl_n_per_m is required for UDL cases")
            if self.udl_n_per_m <= 0:
                raise ValueError("udl_n_per_m must be > 0 for UDL cases")
        return self
